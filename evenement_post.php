		...
		if(!empty($_POST))
	{
					...
					if (isset($_FILES['image']) AND !empty($_FILES['image']['name']))
					{
					    ...
					    if(isset($_POST['titre']) AND (!preg_match('#^[ ]#', $_POST['titre'])))
						{
									...
									{
									  $_SESSION['flash']['danger'] = "veuillez entrer l'heure' : obligatoire";
								      header('Location: evenement.php');
									  exit();
									}
					       ...
					       if (empty($errors))
				           {
							 $req = $bdd->prepare('INSERT INTO evenement (photo, ladate, heure, titre,  details) VALUES (?, ?, ?, ?, ?) ');
							$req->execute([$image, $ladate, $lheure, $titre, $details]);
							 $_SESSION['flash']['success'] = "Ajouter avec success";
							   header('location: evenement.php');
							   exit();
						   }
